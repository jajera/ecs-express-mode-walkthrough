...
for (const [filePath, fileViolations] of byFile) {
  ...
  for (const v of fileViolations) {
    ...
    const sectionRegex = new RegExp(`(## ${v.heading.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[\\s\\S]*?)(?=\\n## |$)`);
    ...
    const termRegex = new RegExp(`(?<!term=["'])(?<!>)\\b(${displayName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})\\b(?!'s<\\/Tooltip)`, 'i');
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (lines[i].match(/^\s*<(Aside|table|thead|tbody|tr|td|th|\/)/)) continue;
      ...
      if (match) {
        lines[i] = lines[i].replace(termRegex, `<Tooltip term="${v.term}">${match[1]}</Tooltip>`);
        fixed = true;
        break;
      }
    }
...
console.log(`Fixed ${violations.length} violations across ${byFile.size} files`);
